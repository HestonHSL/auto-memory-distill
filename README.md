...
- **维护成本**：手动整理项目规范（如 `.frieren/rules/`）繁琐且难以随开发实时同步
...
- Frieren: `.frieren/skills/auto-memory-rules/`

示例：

```bash
mkdir -p .frieren/skills
cp -r auto-memory-rules .frieren/skills/
```
...
```bash
# 生成规则文件（frieren）
bash .frieren/skills/auto-memory-rules/scripts/generate-rules.sh
```

说明：`generate-integration-rules.sh` 仅生成 frieren 集成规则。
...
# 生成目标平台规则（可选，更新规则时运行）
bash scripts/generate-rules.sh            # 生成 frieren 多文件规则
```
...
- `.frieren/rules/memory-integration.md` - Frieren 集成规则（可选生成）
- `.frieren/rules/{category}-rules.md` - Frieren 分类规则（可选生成）
