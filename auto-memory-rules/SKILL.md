...
- 检查技能目录下的 `memory/` 目录（例如：`.frieren/skills/auto-memory-rules/memory/`），不存在则初始化
- 根据用户请求搜索相关记忆并加载到上下文
...
**执行步骤：**
```bash
# 步骤 1：更新平台规则（自动探测并更新 frieren 平台）
...
- Frieren：技能目录 `.frieren/skills/auto-memory-rules/`，规则输出到 `.frieren/rules/`
